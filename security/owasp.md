...
### How It Occurs
Access control issues arise when authorization checks are missing or only validated on the client side. Attackers can manipulate URLs or API calls to access restricted data.

### Example
A user changes the URL:
```
https://app.com/user/123/profile
```
to
```
https://app.com/admin/123/profile
```
and gains admin data because no server-side authorization check exists.
...
- Implement **RBAC (Role-Based Access Control)** via **Azure AD**.
- Apply **Managed Identities** for access to resources.
- Deny access by default and enforce backend checks.
...
### How It Occurs
Sensitive data is stored or transmitted without encryption or with weak algorithms. Often due to using outdated hashing or not securing keys.

### Example
Passwords are stored in plaintext, and HTTPS is not enforced.
...
### Azure-Specific Remediation
- Use **HTTPS/TLS 1.2+** for all traffic.
- Encrypt data at rest using **AES-256**.
- Store secrets in **Azure Key Vault**.
- Use **Managed Identity** for key access.
...
### How It Occurs
Applications concatenate untrusted input directly into queries or system commands.

### Example
An attacker inputs `' OR 1=1--` in a login field, bypassing authentication.

### Vulnerable Code Example
```js
// ❌ Vulnerable SQL Query
const result = db.query("SELECT * FROM users WHERE username = '" + username + "' AND password = '" + password + "'");
```

### Secure Code Example
```js
// ✅ Parameterized Query
const result = db.query('SELECT * FROM users WHERE username = ? AND password = ?', [username, password]);
```

### Azure-Specific Remediation
- Always use **parameterized queries** or ORM.
- Enable **Azure Defender for SQL** to detect injection attempts.
...
### How It Occurs
Poor security decisions are made at the design stage, leading to systemic issues that code cannot fix later.

### Example
An API lacks rate limiting, allowing attackers to brute-force login credentials.

### Remediation
- Perform **threat modeling** early in design.
- Enforce **secure defaults** (e.g., rate limits, session timeouts).
- Use **Azure Application Gateway WAF** to block malicious requests.
...
### How It Occurs
Default credentials, verbose error messages, and exposed configuration files allow attackers to gain insights or control.

### Example
- Application exposes stack traces publicly.
- Storage accounts are left open to public access.

### Vulnerable Code Example
```python
# ❌ Django app with debug mode on
DEBUG = True
```
...
### Azure-Specific Remediation
- Apply **Azure Policy** to enforce hardened configurations.
- Use **Defender for Cloud** to identify risky settings.
- Disable public access to Azure Storage and Key Vault.
...
### How It Occurs
Applications use libraries or frameworks with known vulnerabilities.

### Example
An app uses an outdated **Log4j** version, allowing remote code execution.

### Remediation
- Enable dependency scanning (e.g., **Dependabot**, **Defender for DevOps**).
- Keep libraries up to date.
- Use **Azure DevOps pipeline** tasks to fail builds if vulnerabilities are detected.
